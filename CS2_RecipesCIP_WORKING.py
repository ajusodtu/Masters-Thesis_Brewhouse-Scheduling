...
JI_union = {
        ...
        #Schwarz-bier
        ('MillMash 1', 'MillMashing - xS'):   {'Betamin': 1, 'Betamax': 273, 'gamma': 0.01, 'etamax':30,'theta':10},
        ('MillMash 2', 'MillMashing - xS'):   {'Betamin': 1, 'Betamax': 273, 'gamma': 0.01, 'etamax':30,'theta':10},
        ('Lauter Tun 1', 'Lautering - xS'):   {'Betamin': 1, 'Betamax': 328, 'gamma': 0.01, 'etamax':30,'theta':10},
        ('Lauter Tun 2', 'Lautering - xS'):   {'Betamin': 1, 'Betamax': 328, 'gamma': 0.01, 'etamax':30,'theta':10},
...
for j in J:
    ...
    for t in T:
        ...
        model.con.add(model.F[j,t] <= (etamax[j])*model.W['aCIP',j,t])
        if t >= tgap:
            model.con.add(model.F[j,t] <= etamax[j] - model.F[j,t-tgap])
            model.con.add(model.F[j,t] >= (etamax[j])*model.W['aCIP',j,t] - model.F[j,t-tgap])
        eq = eq - sum([model.W[i,j,t]*theta[j,i] for i in Ij[j]]) + model.F[j,t]
...
model.CIPcon = pyo.Constraint(expr = model.CIP == 50*sum([model.W['aCIP',j,t] for j in J for t in T[26:-26]]))
