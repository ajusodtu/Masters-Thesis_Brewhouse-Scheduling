...
for j in J:
    for t in T:
        model.con.add(sum([model.X[i,j,t] for i in Ij[j]]) == 1)
        model.con.add(sum([model.Y[i,im,j,t] for i in Ij[j] for im in Ij[j]]) <= 1)
        for i in Ij[j]:
            if t >= tau[i]:
            ...
            for im in Ij[j]:
                if i != im:
                    if t >= tgap:
                        model.con.add(model.Y[i,im,j,t] >= model.X[i,j,t-tgap] + model.X[im,j,t] - 1)
                    eq = eq + model.Y[i,im,j,t]
                elif i == im:
                    model.con.add(model.Y[i,im,j,t] == 0)
            if t >= tgap:
                model.con.add(eq <= model.X[i,j,t-tgap])
            model.con.add(eq <= model.X[im,j,t])
